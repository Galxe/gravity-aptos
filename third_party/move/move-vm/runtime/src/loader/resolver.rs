...
use super::{Function, FunctionHandle, LoadedFunction, LoadedFunctionOwner, Module, Script};
...
impl<'a> Resolver<'a> {
    pub(crate) fn for_module(module_storage: &'a dyn ModuleStorage, module: Arc<Module>) -> Self {
    ...
    pub(crate) fn for_script(module_storage: &'a dyn ModuleStorage, script: Arc<Script>) -> Self {
    ...
    pub(crate) fn build_loaded_function_from_handle_and_ty_args(
        &self,
        ...
        match &self.binary {
            BinaryType::Module(module) => {
                let handle = module.function_at(idx.0);
                match handle {
            ...
            BinaryType::Script(script) => {
                let handle = script.function_at(idx.0);
                match handle {
    ...
    pub(crate) fn build_loaded_function_from_instantiation_and_ty_args(
        &self,
