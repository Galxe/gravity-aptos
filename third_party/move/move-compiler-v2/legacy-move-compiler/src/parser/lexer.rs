        ...
        '"' => {
            ...
            match get_string_len(line) {
                ...
                None => {
                    ...
                    return Err(Box::new(diag!(
                        Syntax::InvalidCharacter,
<<<<<<< HEAD:third_party/move/move-compiler/src/parser/lexer.rs
                        (loc, format!("Invalid character: '{}'; string literal must begin with `b\"` and closing quote `\"` must appear on same line", c))
=======
                        (loc, format!("Invalid character: `{}`; string literal must begin with `b\"` and closing quote `\"` must appear on same line", c))
>>>>>>> aptos-node-v1.36.8-hotfix:third_party/move/move-compiler-v2/legacy-move-compiler/src/parser/lexer.rs
                    )));
                },
            }
        },
